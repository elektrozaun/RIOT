...
{
    uint16_t address = reg & 0x0fff;
    uint8_t command = (uint8_t)((reg & 0xf000 ) >> 12) | CMD_READ;
...
{
    uint16_t address = reg & 0x0fff;
    uint8_t command = (uint8_t)((reg & 0xf000 ) >> 12) | CMD_WRITE;
    send_addr(dev, address);
    spi_transfer_byte(dev->p.spi, dev->p.cs, true, command);
...
static void rchunk(w5500_t *dev, uint16_t addr, uint8_t *data, size_t len)
{
    ...
    uint8_t command = (uint8_t)((addr & 0xf800 ) >> 11) | CMD_READ;

    send_addr(dev, address);
    spi_transfer_byte(dev->p.spi, dev->p.cs, true, command);
    for (size_t i = 0; i < (len - 1); i++) {
        data[i] = spi_transfer_byte(dev->p.spi, dev->p.cs, true, 0);
    }
    data[len - 1] = spi_transfer_byte(dev->p.spi, dev->p.cs, false, 0);
}

static void wchunk(w5500_t *dev, uint16_t addr, uint8_t *data, size_t len)
{
    uint16_t address = addr & 0x07ff;
    uint8_t command = (uint8_t)((addr & 0xf800 ) >> 11) | CMD_WRITE;

    send_addr(dev, address);
    spi_transfer_byte(dev->p.spi, dev->p.cs, true, command);
    for (size_t i = 0; i < (len - 1); i++) {
        spi_transfer_byte(dev->p.spi, dev->p.cs, true, data[i]);
    }
    spi_transfer_byte(dev->p.spi, dev->p.cs, false, data[len - 1]);
}
...
void w5500_setup(w5500_t *dev, const w5500_params_t *params, uint8_t index)
{
    ...
    dev->nd.driver = &netdev_driver_w5500;
    ...
    netdev_register(&dev->nd, NETDEV_W5500, index);
}
...
static int init(netdev_t *netdev)
{
    ...
    /* configure 8kB memory to be used by socket 0 */
    wreg(dev, REG_S0_RXBUF_SIZE, RXBUF_8KB_TO_S0);
    wreg(dev, REG_S0_TXBUF_SIZE, TXBUF_8KB_TO_S0);
...
//TODO
static uint16_t tx_upload(w5500_t *dev, uint16_t start, void *data, size_t len)
{
    if ((start + len) >= (S0_TX_BASE + S0_MEMSIZE)) {
        ...
        waddr(dev, REG_S0_TX_WR0, REG_S0_TX_WR1, start + len);
        return (start + len);
    }
}
...
static int send(netdev_t *netdev, const iolist_t *iolist)
{
    ...
    uint16_t tx_wr = raddr(dev, REG_S0_TX_WR0, REG_S0_TX_WR1);
    ...
    waddr(dev, REG_S0_TX_WR0, REG_S0_TX_WR1, tx_wr + sum);

    /* trigger the sending process */
    wreg(dev, REG_S0_CR, CR_SEND_MAC);
    while (!(rreg(dev, REG_S0_IR) & IR_SEND_OK)) {}
    wreg(dev, REG_S0_IR, IR_SEND_OK);
...
//TODO
static inline void drop(w5500_t *dev, uint16_t num, uint16_t rp, uint16_t psize)
{
    /* set the new read pointer address */
    waddr(dev, REG_S0_RX_RD0, REG_S0_RX_RD1, rp + psize);
    wreg(dev, REG_S0_CR, CR_RECV);

    /* if RX buffer now empty, clear RECV interrupt flag */
    if ((num - psize) == 0) {
        wreg(dev, REG_S0_IR, IR_RECV);
    }
}
...
static int recv(netdev_t *netdev, void *buf, size_t max_len, void *info)
{
    ...
    uint16_t num = raddr(dev, REG_S0_RX_RSR0, REG_S0_RX_RSR1);

    if (num > 0) {
        /* find the size of the next packet in the RX buffer */
        uint16_t rp = raddr(dev, REG_S0_RX_RD0, REG_S0_RX_RD1);
        uint16_t psize = raddr(dev, (S0_RX_BASE + (rp & S0_MASK)),
                               (S0_RX_BASE + ((rp + 1) & S0_MASK)));
        len = psize - 2;

        DEBUG("[w5500] recv: got packet of %i byte (at 0x%04x)\n", len,
              (int)rp);
        ...
        if (in_buf != NULL) {
            ...
            DEBUG("[w5500] recv: read %i byte from device (at 0x%04x)\n",
                  len, (int)rp);
        }
